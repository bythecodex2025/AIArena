...
export function formatBattleLog(log) {
  const lines = [
    "Relay Forge AI 대전 로그",
    "========================",
    `규칙: relay-forge / ${log.rulesVersion}`,
    `경기: #${log.sequence}`,
    `상대: ${log.opponentName} (${log.opponent})`,
    `내 AI 진영: ${log.customSide}`,
    `코드 파일: ${log.codeFileName || "my-relay-ai.js"}`,
    `결과: ${LABELS[log.result] ?? log.result}`,
    `종료 사유: ${log.reason || "—"}`,
    `총 반수: ${log.plies}`,
    `제한: ${log.timeoutMs}ms/행동 · 최대 ${log.maxPlies}반수`,
    `시각: ${log.playedAt}`,
    "",
    "행동",
    "----",
  ...
  for (const action of log.actions) {
    ...
    lines.push(`${action.ply}. ${action.side} ${action.actor} · ${action.text}${timing} · E ${action.energy} · R ${action.relays} · SCORE ${action.score ?? action.streak ?? 0}`);
  }
...
export function battleLogFilename(log) {
  ...
  return `relay-forge-${opponent}-game-${String(log.sequence).padStart(2, "0")}-${log.result}.txt`;
}
